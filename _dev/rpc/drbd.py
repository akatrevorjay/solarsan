...
class DrbdPeer(object):
    ...
    @property
    def is_local(self):
        return self.hostname == settings.SERVER_NAME


class DrbdReplication(object):
    name = None
    shared_secret = None
    sync_rate = None
    peers = None
    ...
    def __init__(self, **kwargs):
        ...
        self.sync_rate = kwargs.get('sync_rate')
        self.peers = kwargs.get('peers')

        #if kwargs.get('random_secret'):
        #    self.generate_random_secret()

    def generate_random_secret(self):
        self.shared_secret = str(random.getrandbits(128))
        return True

    def write_config(self):
        context = self.__dict__.copy()
        return config_from_template('drbd-resource.template', context=context, is_file=True)


def _test_carp():
    name = 'r50'
    minor = 10
    tmp = DrbdReplication(name=name)
    tmp.peers = [
        DrbdPeer(hostname="san0", minor=minor, disk="/dev/zvol/dpool/%s" % name, address="10.90.90.165:7800"),
        DrbdPeer(hostname="san1", minor=minor, disk="/dev/zvol/dpool/%s" % name, address="10.90.90.151:7800"),
    ]
    return tmp
...
def _parse_proc_drbd(lines):
    ...
    version = m and m.group(1) or None
    minors = {}
    minor = None
    device = {}
    for line in lines[2:]:
...
class _proc_drbd_test(unittest.TestCase):
...
from drbdadm import util

class Drbd:
    """Represents the real drbd system"""
    def _read_proc_drbd():
        return _proc_drbd(util.read_file("/proc/drbd"))
...
import losetup, os
from drbdadm.util import run, CommandError, log
class Localdevice:
